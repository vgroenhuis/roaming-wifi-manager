...
const char WIFI_HTML[] PROGMEM = R"MyString(<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .connected-row td { border-left: 4px solid #28ff45; }
        .same-ssid-row td { border-left: 4px solid #008000; }
...
<body>
    <div class="container">
        ...
        <div class="settings-panel">
            ...
            <div class="settings-row">
                ...
                <input class="settings-checkbox" type="checkbox" id="autoRescanSkipNotDetectedToggle" onchange="updateAutoScanSetting()">
                <span>Skip non-detected</span>
    ...
    <script>
        try {
        console.log('[WiFiPage] Script execution started');
        ...
        function fetchBssidAliasesIfConfigured() {
            console.log('[WiFiPage] fetchBssidAliasesIfConfigured called');
        ...
        function clampWaitIntervalSec(value, fallback) {
            const n = Number(value);
            if (!Number.isFinite(n)) return fallback;
            // Keep 0.01s (10ms) resolution to match input step.
        ...
        function getSeriesColorForNetwork(net) {
            // Use backend-provided attributes for accurate color determination
            const connected = !!(net && net.connected);
            if (connected) return '#28a745'; // Bright green for connected

            const sameSsidAsConnected = !!(net && net.sameSsidAsConnected);
            if (sameSsidAsConnected) return '#006400'; // Dark green for same SSID

            // Known network with different SSID
            const known = !!(net && net.known);
            if (known) return '#007bff'; // Blue for other known networks

            return '#b9bec7'; // Gray for unknown networks
        }
        ...
        function updateSignalHistoryFromNetworks(nowMs) {
            ...
            for (const net of networksData) {
                ...
                if (!last || (nowMs - last.t) >= 500) {
                    ...
                    // Store color with each point so historical colors are retained
                    const color = getSeriesColorForNetwork(net);
                    pts.push({ t: nowMs, rssi: rssi, gap: gap, color: color });
                    series.hadGap = false;
                }
            }
        ...
        function drawSignalGraph() {
            ...
            for (const net of (networksData || [])) {
                ...
                // 2+ points: draw line segments using stored color from each point.
                ...
                for (let i = 1; i < pointsInWindow.length; i++) {
                    ...
                    // Use color from the destination point (p1) for the segment, unless there's a gap
                    const segmentColor = (p1 && p1.gap) ? undetectedColor : (p1.color || '#b9bec7');
                ...
                <table class="networks-table">
                    ...
                    <tbody>
                        ${rows.map(network => {
                            ...
                            // Check if same SSID as currently connected but different BSSID
                            const networkSSID = (network && network.ssid) ? String(network.ssid) : '';
                            const sameSsid = currentSSID && networkSSID && (String(currentSSID) === networkSSID) && !connected;
                            const baseClass = connected ? 'connected-row' : (sameSsid ? 'same-ssid-row' : (known ? 'known-row' : 'unknown-row'));
        ...
        function setAutoRescanSkipNotDetectedFromServer(skipNotDetected) {
            const toggle = document.getElementById('autoRescanSkipNotDetectedToggle');
            if (toggle) toggle.checked = !!skipNotDetected;
        }
        ...
        function setAutoRescanWaitIntervalFromServer(intervalSec) {
            const input = document.getElementById('autoRescanWaitInterval');
            if (input) input.value = String(clampWaitIntervalSec(intervalSec, 0));
        }

        console.log('[WiFiPage] Defining updateAutoScanSetting function');
        function updateAutoScanSetting() {
            console.log('[WiFiPage] updateAutoScanSetting called');
            ...
            const rescanSkipNotDetectedToggle = document.getElementById('autoRescanSkipNotDetectedToggle');
            const rescanTestChannelsToggle = document.getElementById('autoRescanTestChannelsToggle');
            const rescanWaitInput = document.getElementById('autoRescanWaitInterval');
            if (!fullToggle || !fullInput || !rescanToggle || !rescanInput || !rescanKnownOnlyToggle || !rescanSkipNotDetectedToggle || !rescanTestChannelsToggle || !rescanWaitInput) return;
            ...
            const rescanSkipNotDetected = !!rescanSkipNotDetectedToggle.checked;
            const rescanTestChannels = !!rescanTestChannelsToggle.checked;

            const rescanWaitIntervalSec = clampWaitIntervalSec(rescanWaitInput.value, 0);
            rescanWaitInput.value = String(rescanWaitIntervalSec);
            ...
            authenticatedFetch('/wifi/autoscan', {
                ...
                body: JSON.stringify({
                    ...
                    rescanSkipNotDetected: rescanSkipNotDetected,
                    rescanTestChannels: rescanTestChannels,
                    rescanWaitIntervalSec: rescanWaitIntervalSec,
                })
            })
            .then(response => response.json())
            .then(data => {
                ...
                setAutoRescanSkipNotDetectedFromServer(data.rescanSkipNotDetected ?? rescanSkipNotDetected);
                setAutoRescanTestChannelsFromServer(data.rescanTestChannels ?? rescanTestChannels);
                setAutoRescanWaitIntervalFromServer(data.rescanWaitIntervalSec ?? rescanWaitIntervalSec);
            })
            .catch(() => {
                ...
                setAutoRescanWaitIntervalFromServer(rescanWaitIntervalSec);
            });
        }

        console.log('[WiFiPage] Defining updateAutoScanToggle function');
        function updateAutoScanToggle() {
            console.log('[WiFiPage] updateAutoScanToggle called');
            authenticatedFetch('/wifi/settings')
                ...
                .then(data => {
                    ...
                    const rescanSkipNotDetected = data.autoRescanSkipNotDetected ?? true;
        ...
        console.log('[WiFiPage] Defining loadGraphSettings function');
        function loadGraphSettings() {
            console.log('[WiFiPage] loadGraphSettings called');
            try {
        ...
        // Initial load
        console.log('[WiFiPage] Starting initialization sequence');
        try {
            console.log('[WiFiPage] Calling loadGraphSettings()');
            loadGraphSettings();
            console.log('[WiFiPage] Calling updateAutoScanToggle()');
            updateAutoScanToggle();
            console.log('[WiFiPage] Calling refreshStatusAndNetworks()');
            refreshStatusAndNetworks();
            console.log('[WiFiPage] Initialization complete');
        } catch (error) {
            console.error('[WiFiPage] Initialization error:', error);
        }
        } catch (scriptError) {
            console.error('[WiFiPage] FATAL: Script parsing/execution error:', scriptError);
        }
    </script>
</body>
</html>
